...
fit <- train(FraudFound_P~., data = transformed, trControl = control, method = "nb", metric = "Accuracy", na.action = na.omit)
fit
...
set.seed(7)
fit <- train(FraudFound_P~., data = transformed, trControl = control, method = "glm", metric = "Accuracy", na.action = na.omit)
fit
#predictions <- predict(fit, dplyr::select(transformed, -FraudFound_P))
#confusionMatrix(predictions, transformed$FraudFound_P)
